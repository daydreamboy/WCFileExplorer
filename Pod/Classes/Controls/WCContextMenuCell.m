...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    ...
    if (self.contextMenuItemTypes.count) {
        ...
        else if (action == @selector(deleteAction:) && (self.contextMenuItemOptions & WCContextMenuItemDeletion)) {
            return YES;
        }
        else {
            return NO;
...
- (NSArray *)customMenuItems {
    ...
    for (NSUInteger i = 0; i < [self.contextMenuItemTypes count]; i++) {
        ...
        else if (option & WCContextMenuItemDeletion) {
...
- (void)deleteAction:(id)sender {
    if (self.allowCustomActionContextMenuItems & WCContextMenuItemDeletion) {
        if ([self.delegate respondsToSelector:@selector(contextMenuCell:contextMenuItemClicked:withSender:)]) {
            [self.delegate contextMenuCell:self contextMenuItemClicked:WCContextMenuItemDeletion withSender:self];
        }
    }
    else {
        // Default action
